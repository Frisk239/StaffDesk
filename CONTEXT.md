...
- M33 built the recovery and observability floor (fourth-audit F1/F2/F3/F7): a corrupt brain no longer shows as double-click-dead — startup catches openBrain, quarantines the file as brain.db.corrupt-<ts> (WAL sidecars cleaned, secrets/backups untouched), shows a native error box with the two ways out, and exits after a render delay; load-bearing fix found on the way — openDatabase leaked the SQLite handle on pragma errors, which blocked the quarantine rename on Windows (close-on-fail added). Module-load-time handlers log uncaughtException/unhandledRejection/render-process-gone without rethrowing, and a kill-and-reopen e2e proves ledger consistency after a hard kill. A hand-rolled redact-masked log (daily files, 5MB rotation, 30-day sweep — chosen over electron-log to add no runtime dependency) replaces the console.warns with logs:dir/logs:export trusted IPC and a Settings 诊断 section; Settings also gained a memory section browsing global/object/session memories with the ban triple and per-kind removal toasts. The CI package job now silent-installs the NSIS build (/S), asserts the process lives and brain.db is created, and force-cleans the tray-resident process; the markdown.tsx copy button rides the trusted clipboard IPC (paying M32's booked debt). ADR 0063 settled operations retention (row cap with evidence exemptions, implementation M34).
- M34 closed the consolidation phase (fourth-audit D2/D3/D6/F5/F6 + ADR 0063 implementation): applyAction.ts went from 3293 lines to an 86-line dispatch shell over seven domain files plus shared helpers — a pure relocation certified by full mechanical diff (66 case blocks and 39 helpers verbatim; recursive dispatch reaches the shell only through an injected reducer parameter), with three behavioral suspicions reported-not-fixed per the knife's red line. operations gained the 0063 retention (20k row cap pruning oldest non-exempt, with DELETE_SOURCE/correction/role-change rows forever kept, count-probed and applied after append — the per-row insert model makes post-append pruning equivalent to the ADR's intent). The renderer test layer went from zero to real: happy-dom + testing-library with per-file environments, first component tests for the memory section, chat failure toast/busy states, and predicate-table interactions (review backfill), all mocked at the window.staffdesk IPC boundary; a chat-failure e2e injects connection loss by closing the stub server; and a scale baseline times 5k-claim snapshots and dispatches at both the in-memory reducer and full-persistence layers with order-of-magnitude thresholds. e2e duration trend recording resumes (M34: 38 specs, 18.6s).
- M36 built the closeout face (fifth-audit G1/G2/G3/G5 + head-car fixes): the repo gained its README (positioning, core concepts, quickstart with Node 22 matching CI, an architecture sketch, quality numbers traceable to gate outputs, ten curated ADRs, and a docs tour), an MIT LICENSE, a CI badge, and dependabot watching both package.json files plus the actions; the package job now publishes a GitHub Release on tag pushes (contents:write granted at job level, gh CLI preinstalled on the runner). The fifth audit's findings were paid: the 0063 sort key really works now (SELECT aliases created_at to the interface's camelCase — the as-cast had left it undefined, degrading to id order; a real-DB test with inverted id/time ordering fails on the degraded implementation), two dead code paths went away (the unreachable fromUrl title fallback and SET_VIEW's self-assignment), brief export failures now surface as a toast instead of vanishing (user cancels stay silent), the 简报 entry gained its primary-source marking and Markdown export wording, the installer smoke probes brain.db under both userData candidates (the real landing is staffdesk-app — Electron uses the package name, a mismatch recorded but left alone since renaming migrates user data), and the brief-export spec's async assertions carry 15s windows after three different slow-runner ghost failures. F1's landing spot was pinned by the #31 merge run's diagnostics after the probe succeeded.
- Current working branch for this cut: `codex/m36-closeout-face`. Previous merged branch: `codex/m35-audit-round-5`.
...
_Avoid_: 草稿, 待生成, 低分来源, 主张状态的一种, 滞留（滞留是未核之上的整理条件）

**滞留**:
成立且未核的主张，自入库起已满设定天数（默认 7 天，设置可改）。整理可提议丢弃，人确认才删。不是主张状态，不是过时。
_Avoid_: 过时, 未核（未核是核对标志）, 立即清理, 对象级闲置
...
**对象页**:
当前对象的工作面。主栏是挂在该对象下的对话；档案投影、已绑定来源、简报按需在右栏打开。右栏只读：来源解绑、删除、重试抽取与设角色一样，右栏入口只打开主栏确认卡。投影仍是主张按谓词槽编，冲突并排，未知空着，不准出现账本里没有的新判断。顶栏办事：调研、出简报、任务回放。
_Avoid_: wiki, 档案正文, compiled truth, 简介, 无对象的首页对话, 把档案做成主表, 右栏直接改账本
...
**简报**:
挂在对象和任务下的出站快照，不是第四种对象。当时账本里能出站的主张，带着引用、未知和未核标记；主张出自主键绑定时带「主键来源」标注（0062），可整篇复制或导出 Markdown（引用转脚注）。对象页继续长；同一对象上可留多份历史简报。
...
**整理**:
任务后或空闲时的提议：合并重复主张、补关系、标可能过时、给未编目谓词编目、丢弃滞留的未核、提议建新对象。丢弃卡按滞留集合刷新，接受时按 live 重算；打开待确认会扫描一遍。人确认才改账本。不挡进料。不是 dream。
_Avoid_: 静默改写, dream, 记忆整理, 夜间（本机形态应用没开就不跑）, 抽取后立即丢弃未核
